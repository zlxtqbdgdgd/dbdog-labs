...
另外三个 label 不归你写，写了会被拒：`finding_kinds`（改进点类别，import 从 `findings` 算）、
`fix_marks`（修的人打的标记）、`rubric_version`（**判的是哪一版口径**，import 从包里记的那份写）。

最后那个尤其别自己填：开跑前守门会逐例同步 plugin，你手上这份 rubric 是哪一版，你自己多半说不准，
而填错了比不填更坏——跨轮比较时它正是用来分清「判官变了」还是「口径变了」的。
